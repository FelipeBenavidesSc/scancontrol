...
static public function listarTelectricovdfMdl($tabla) {
	...
	static public function listarTelectricoenchufeMdl($tabla) {
		$sql = Conexion::conectar()->prepare("SELECT * FROM $tabla ");
		$sql -> execute();
		return $sql -> fetchAll();
	}
		static public function listarTelectricoprensasMdl($tabla) {
		$sql = Conexion::conectar()->prepare("SELECT * FROM $tabla ");
		$sql -> execute();
		return $sql -> fetchAll();
	}
	static public function mdlCrearTableroelectrico($tabla, $datos,$rutaImagen) {
		$sql = Conexion::conectar()->prepare("INSERT INTO $tabla() VALUES (NULL, :altura,:ancho,:fondo,:contactor,:marcaswitch,:puertoswitch,:modeloemergencia,:botoneras,:mainswitch,:imagen)");
		...
		$sql->bindParam(":marcaswitch", $datos["marcaswitch"], PDO::PARAM_STR);
		$sql->bindParam(":puertoswitch", $datos["puertoswitch"], PDO::PARAM_STR);
		$sql->bindParam(":modeloemergencia", $datos["modeloemergencia"], PDO::PARAM_STR);
		$sql->bindParam(":botoneras", $datos["botoneras"], PDO::PARAM_STR);
		$sql->bindParam(":mainswitch", $datos["mainswitch"], PDO::PARAM_STR);
...
$enchufes=count($datos["enchufemodelo"]);
$enchufemodelo=$datos["enchufemodelo"];
$enchufeamperaje=$datos["enchufeamperaje"];

if($enchufes >=1){
for ($i=0; $i <$enchufes ; $i++) { 

if ($enchufemodelo[$i]!='') {

$sqlenchufes = Conexion::conectar()->prepare("INSERT INTO telectrico_enchufes(id_tablero_electrico,modelo,amperaje) VALUES('". $id_tablero."','".$enchufemodelo[$i]."','".$enchufeamperaje[$i]."')");
$sqlenchufes -> execute();
}
}
}

$prensas=count($datos["prensasmodelo"]);
$prensasmodelo=$datos["prensasmodelo"];
$prensascantidad=$datos["prensascantidad"];
...
$sqlprensas = Conexion::conectar()->prepare("INSERT INTO telectrico_prensas(id_tablero_electrico,modelo,cantidad) VALUES('". $id_tablero."','".$prensasmodelo[$i]."','".$prensascantidad[$i]."')");
$sqlprensas -> execute();
}
}
}
...
$estado=true;
	...
	static public function mdlEliminarTenchufe($tabla, $id_tenchufe) {
		$sql = Conexion::conectar()->prepare("DELETE FROM $tabla WHERE id_telectrico_enchufes = :id");
		$sql->bindParam(":id", $id_tenchufe, PDO::PARAM_INT);
		if( $sql->execute()) {
			return "ok";
		} else {
			return "error";
	...
	static public function mdlEliminarTprensas($tabla, $id_tprensas) {
		$sql = Conexion::conectar()->prepare("DELETE FROM $tabla WHERE id_telectrico_prensas = :id");
		$sql->bindParam(":id", $id_tprensas, PDO::PARAM_INT);
		if( $sql->execute()) {
			return "ok";
		} else {
			return "error";
	...
	static public function mdlEditarTableroelectrico($tabla, $id_tableroelectrico) {
		...
		return $sql -> fetchAll();
			...
			static public function mdlEditarTableroenchufe($tabla, $id_tableroelectrico) {
		$sql = Conexion::conectar()->prepare("SELECT * FROM $tabla t WHERE t.id_tablero_electrico = :id");
		$sql->bindParam(":id", $id_tableroelectrico, PDO::PARAM_INT);
		$sql -> execute();
		return $sql -> fetchAll();
	}
			static public function mdlEditarTableroprensas($tabla, $id_tableroelectrico) {
		$sql = Conexion::conectar()->prepare("SELECT * FROM $tabla t WHERE t.id_tablero_electrico = :id");
		$sql->bindParam(":id", $id_tableroelectrico, PDO::PARAM_INT);
		$sql -> execute();
		return $sql -> fetchAll();
	...
	static public function mdlActualizarTableroelectrico($tabla, $datos,$rutaImagen) {
		if( is_null($rutaImagen)) {
			$sql = Conexion::conectar()->prepare("UPDATE $tabla SET altura = :altura,ancho =:ancho,fondo =:fondo, contactor = :contactor, marcaswitch = :marcaswitch, puertoswitch = :puertoswitch, modeloemergencia = :modeloemergencia, botoneras = :botoneras, mainswitch = :mainswitch WHERE id_tableroelectrico = :id");
			$sql->bindParam(":altura", $datos["altura"], PDO::PARAM_STR);
			$sql->bindParam(":ancho", $datos["ancho"], PDO::PARAM_STR);
			$sql->bindParam(":fondo", $datos["fondo"], PDO::PARAM_STR);
			$sql->bindParam(":contactor", $datos["contactor"], PDO::PARAM_STR);
		...
		$sql = Conexion::conectar()->prepare("UPDATE $tabla SET altura = :altura,ancho =:ancho,fondo =:fondo, contactor = :contactor,marcaswitch = :marcaswitch, puertoswitch = :puertoswitch, modeloemergencia = :modeloemergencia, botoneras = :botoneras, mainswitch = :mainswitch, rutaImg = :rutaNueva WHERE id_tableroelectrico = :id");
		$sql->bindParam(":altura", $datos["altura"], PDO::PARAM_STR);
			$sql->bindParam(":ancho", $datos["ancho"], PDO::PARAM_STR);
			$sql->bindParam(":fondo", $datos["fondo"], PDO::PARAM_STR);
			$sql->bindParam(":contactor", $datos["contactor"], PDO::PARAM_STR);
...
$enchufe=count($datos["idenchufe"]);
if($enchufe >=1){
for ($i=0; $i <$enchufe ; $i++) { 
$idenchufe=$datos['idenchufe'][$i];
$enchufemodelo=$datos['enchufemodelo'][$i];
$enchufeamperaje=$datos['enchufeamperaje'][$i];



$sqlenchufes = Conexion::conectar()->prepare("UPDATE telectrico_enchufes SET modelo = '$enchufemodelo',amperaje = '$enchufeamperaje' WHERE id_telectrico_enchufes = '".$idenchufe."'");
$sqlenchufes -> execute();
}
}
$enchufeeditar=count($datos["enchufemodeloeditar"]);
$enchufemodeloeditar=$datos["enchufemodeloeditar"];
$enchufeamperajeeditar=$datos["enchufeamperajeeditar"];
if($enchufeeditar >=1){
for ($i=0; $i <$enchufeeditar ; $i++) { 

$sqlenchufeeditar = Conexion::conectar()->prepare("INSERT INTO telectrico_enchufes(id_tablero_electrico,modelo,amperaje) VALUES('". $id_tableroelectrico."','". $enchufemodeloeditar[$i]."', '".$enchufeamperajeeditar[$i]."')");
$sqlenchufeeditar -> execute();
}
}

$prensas=count($datos["idprensas"]);
if($prensas >=1){
for ($i=0; $i <$prensas ; $i++) { 
$idprensas=$datos['idprensas'][$i];
$prensasmodelo=$datos['prensasmodelo'][$i];
$prensascantidad=$datos['prensascantidad'][$i];



$sqlprensas = Conexion::conectar()->prepare("UPDATE telectrico_prensas SET modelo = '$prensasmodelo',cantidad = '$prensascantidad' WHERE id_telectrico_prensas = '".$idprensas."'");
$sqlprensas -> execute();
}
}

$prensaseditar=count($datos["prensasmodeloeditar"]);
$prensasmodeloeditar=$datos["prensasmodeloeditar"];
$prensascantidadeditar=$datos["prensascantidadeditar"];



if($prensaseditar >=1){
for ($i=0; $i <$prensaseditar ; $i++) { 

$sqlprensaseeditar = Conexion::conectar()->prepare("INSERT INTO telectrico_prensas(id_tablero_electrico,modelo,cantidad) VALUES('". $id_tableroelectrico."','". $prensasmodeloeditar[$i]."', '".$prensascantidadeditar[$i]."')");
$sqlprensaseeditar -> execute();
}
}
