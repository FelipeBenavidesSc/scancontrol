...
static public function listarGraderMdl($tabla) {
		$sql = Conexion::conectar()->prepare("SELECT a.id_unidad_balanza,a.cliente,a.grader,a.id_unidad_al,a.id_unidad_acel,a.id_unidad_desc,a.id_calidad,a.id_pesaje,e.descripcion descripcalidad,e.rutaImg imgcalidad,al.descripcion descralim,al.rutaImg desimg,acel.descripcion descraceleracion,pes.descripcion descrpesaje,descar.descripcion descripdescarga FROM $tabla a LEFT JOIN estacion_calidad e on e.id_calidad=a.id_calidad LEFT JOIN unidad_alim al on al.id_unidad_alim=a.id_unidad_al LEFT JOIN unidad_acel acel on acel.id_unidad_acel=a.id_unidad_acel LEFT JOIN unidad_pesaje pes on pes.id_unidad_pesaje=a.id_pesaje LEFT JOIN unidad_descarga descar on descar.id_unidad_descarga=a.id_unidad_desc ");
		$sql -> execute();
		return $sql -> fetchAll();
}
