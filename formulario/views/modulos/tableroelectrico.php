...
<table id="example" class="table table-striped table-bordered" style="width:70%">
        <thead>
            <tr style="font-size:12px;">
                ...
                <th>Modelo Main Switch</th>

                <th>Modelo Parada Emergencia</th>
                <th>Botoneras Partir/Parar</th>
                <th>Enchufes</th>
                <th>Prensas</th>
...
echo '<td>'.nl2br($value["marcaswitch"]).'</td>';
echo '<td>'.nl2br($value["puertoswitch"]).'</td>';
echo '<td>'.nl2br($value["mainswitch"]).'</td>';
echo '<td>'.nl2br($value["modeloemergencia"]).'</td>';
echo '<td>'.nl2br($value["botoneras"]).'</td>';

echo '<td>';
  $tablaenchufe = ControllerTableroelectrico::listarTelectricoenchufeCtr();
foreach ($tablaenchufe as $key => $valor) {
$idtablaenchufe=$valor["id_tablero_electrico"];
if ($idtablaenchufe==$id_tableroelectrico) {


   echo '<div class="shadow-lg p-3 mb-5 rounded" style="background:#f3f6fb;margin-bottom:2px;"><div class="row"><div class="col-sm-12"><label style="font-weight: bold;">Modelo:</label> '.$valor["modelo"].' </div><div class="col-sm-12"> <label style="font-weight: bold;">Amperaje:</label> '.$valor["amperaje"].'</div></div></div>';
}
}
echo '</td>';
echo '<td>';
  $tablaprensas = ControllerTableroelectrico::listarTelectricoprensasCtr();
foreach ($tablaprensas as $key => $valor) {
$idtablaprensas=$valor["id_tablero_electrico"];
if ($idtablaprensas==$id_tableroelectrico) {


   echo '<div class="shadow-lg p-3 mb-5 rounded" style="background:#f3f6fb;margin-bottom:2px;"><div class="row"><div class="col-sm-12"><label style="font-weight: bold;">Modelo:</label> '.$valor["modelo"].' </div><div class="col-sm-12"> <label style="font-weight: bold;">Cantidad:</label> '.$valor["cantidad"].'</div></div></div>';
}
}
echo '</td>';
echo '<td>';
...
<td><img width="250" src="'.substr($value["rutaImg"], 3).'"></td>
